...
function ParentDashboard() {
  ...
  const [editChildForm, setEditChildForm] = useState({ id: '', name: '', email: '', password: '' });
  ...
  const loadChildren = async () => {
    ...
    try {
      const childrenData = await apiRequest(`/api/children/parent/${parentId}`);
      syncChildrenFromBackend(childrenData);
    ...
    const transactions = await getRecentTransactions(parentId);
  ...
  useEffect(() => {
    ...
    const refreshFromLocal = async () => {
      const nextChildren = getChildrenByParent(parentId);
      setChildren(nextChildren);
      setSelectedChild((current) => (
        current ? nextChildren.find((child) => child.id === current.id) || current : current
      ));
      setLoanChild((current) => (
        current ? nextChildren.find((child) => child.id === current.id) || current : current
      ));
      setRecentTransactions(await getRecentTransactions(parentId));
    };
  ...
  const handleAddChild = async () => {
    ...
    // 자녀 계정도 users에 추가
    try {
      const data = await apiRequest('/api/children', {
        method: 'POST',
        body: JSON.stringify({
          name: addForm.name,
          email: addForm.email,
      ...
      if (data.child) syncChildFromBackend(data.child);

      await loadChildren();
      setNewChildInfo({ email: addForm.email, password: addForm.password });
      setAddForm({ name: '', email: '', password: '' });
      setOpenAddDialog(false);
      setSnackbar({
        open: true,
        message: `${data.child?.name || addForm.name} 자녀 계정이 연결되었습니다.`,
        severity: 'success',
      });
      return;
    } catch (apiError) {
    ...
    const existingUser = users.find(u => u.email?.toLowerCase() === addForm.email.toLowerCase());
    if (existingUser) {
      try {
        const linked = linkChildToParent(existingUser.id, parentId);
        setChildren(getChildrenByParent(parentId));
        setNewChildInfo({ email: linked.user.email, password: addForm.password });
        setAddForm({ name: '', email: '', password: '' });
        setOpenAddDialog(false);
        setSnackbar({
          open: true,
          message: `${linked.child.name} 자녀 계정이 연결되었습니다.`,
          severity: 'success',
        });
      } catch (linkError) {
    ...
    setSnackbar({
      open: true,
      message: `${addForm.name} 자녀 계정이 추가되었습니다.`,
      severity: 'success',
    });
  };
  ...
  const handleApproveLoan = (request) => {
    ...
    addNotification(updated.id, {
      type: 'loan',
      title: '대출이 승인되었어요',
      message: `${Number(request.amount).toLocaleString()}원이 입금되었습니다.`,
      dedupeKey: `loan-approved:${updated.id}:${request.id}`,
      data: { childId: updated.id, loanId: request.id },
    });
    setChildren(getChildrenByParent(parentId));
  ...
  const handleRejectLoan = (request) => {
    ...
    addNotification(updated.id, {
      ...
      dedupeKey: `loan-rejected:${updated.id}:${request.id}`,
      data: { childId: updated.id, requestId: request.id },
    });
    setChildren(getChildrenByParent(parentId));
    setOpenLoanDialog(false);
  };
  ...
  const handleRepayLoan = (loan, child) => {
    ...
    addNotification(updated.id, {
      ...
      dedupeKey: `loan-repaid-child:${updated.id}:${loan.id}`,
      data: { childId: updated.id, loanId: loan.id },
    });
    setChildren(getChildrenByParent(parentId));
    setOpenLoanDialog(false);
  };
  ...
  // 상세보기
  const handleSendAllowancePersisted = async () => {
    if (!sendAmount || isNaN(sendAmount) || Number(sendAmount) <= 0 || !selectedChild) return;

    let updated;
    let savedByBackend = false;
    try {
      const data = await apiRequest(`/api/children/${selectedChild.id}/allowance`, {
        method: 'POST',
      ...
      if (!isNetworkError(apiError)) {
        setSnackbar({ open: true, message: apiError.message, severity: 'error' });
        return;
      }

      updated = {
        ...selectedChild,
        balance: (selectedChild.balance || 0) + Number(sendAmount),
        ledgers: [
          {
            type: '입금',
            amount: Number(sendAmount),
          ...
          ...(selectedChild.ledgers || []),
        ],
      };
    }

    if (savedByBackend) {
      syncChildFromBackend(updated);
    } else {
      updateChild(updated);
      addNotification(updated.id, {
      type: 'allowance',
      title: '용돈이 도착했어요',
      message: `${Number(sendAmount).toLocaleString()}원이 입금되었습니다.`,
      dedupeKey: `allowance:${updated.id}:${updated.ledgers?.[0]?.id || Date.now()}`,
      data: {
        childId: updated.id,
        amount: Number(sendAmount),
        ledgerId: updated.ledgers?.[0]?.id,
      },
      });
    }
    await loadChildren();
    ...
    setOpenSendDialog(false);
    setSendAmount('');
    setSnackbar({ open: true, message: `${updated.name}에게 용돈을 보냈습니다.`, severity: 'success' });
  };
  ...
  const handleSaveEditChild = async () => {
    if (!editChildForm.name || !editChildForm.email) {
      setSnackbar({ open: true, message: '이름과 이메일을 입력해 주세요.', severity: 'error' });
      return;
    }

    try {
      const data = await apiRequest(`/api/children/${editChildForm.id}`, {
        method: 'PUT',
        body: JSON.stringify(editChildForm),
      });
      syncChildFromBackend(data);
      await loadChildren();
      setOpenEditDialog(false);
      setSnackbar({ open: true, message: '자녀 정보가 수정되었습니다.', severity: 'success' });
    } catch (apiError) {
      if (!isNetworkError(apiError)) {
        setSnackbar({ open: true, message: apiError.message, severity: 'error' });
        return;
  ...
  const handleDeleteChild = async (childId) => {
    try {
      await apiRequest(`/api/children/${childId}`, { method: 'DELETE' });
    } catch (apiError) {
      if (!isNetworkError(apiError)) {
        setSnackbar({ open: true, message: apiError.message, severity: 'error' });
        return;
    ...
    await loadChildren();
    setSnackbar({ open: true, message: '자녀 계정이 삭제되었습니다.', severity: 'success' });
  };
    ...
    <Box sx={{ 
        ...
        <DialogContent>
          <Box sx={{ mt: 2 }}>
            ...
            <TextField
              ...
              label="비밀번호"
              type="password"
              value={editChildForm.password}
              onChange={(e) => setEditChildForm({ ...editChildForm, password: e.target.value })}
            />
          </Box>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={openSendDialog} onClose={() => setOpenSendDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          ...
          <Button
            variant="contained"
            onClick={handleSendAllowancePersisted}
            sx={{
              bgcolor: '#FFD600',
              color: '#222',
              '&:hover': {

...
const ChildDashboard = () => {
  ...
  useEffect(() => {
    if (user?.role === 'child') {
      const loadLatestChild = async () => {
        let latest = findChildById(user.id);
        try {
          latest = await apiRequest(`/api/children/${user.id}`);
          syncChildFromBackend(latest);
        } catch (apiError) {
  ...
  useEffect(() => {
    if (user?.role !== 'child' || !user?.id) return;
    ...
    const reloadChild = async () => {
      ...
      setStats(await getChildStats(user.id));
      setRecentTransactions(await getRecentTransactions(user.id));
    };
  ...
  const remainingMissions = missionsList.filter(m => m.status === '진행중' || m.status === '승인대기').length;
  const progressTargetMissions = missionsList.filter(m => m.status !== '거부').length;
  ...
  const handleRepayLoan = (loan) => {
    ...
    updateChild(updated);
    if (updated.parentId) {
      (typeof navigator === 'undefined' || !navigator.onLine) && addNotification(updated.parentId, {
        type: 'loan',
        title: '대출 상환 완료',
        message: `${updated.name}님이 대출을 상환했습니다.`,
        dedupeKey: `loan-repaid-parent:${updated.id}:${loan.id}`,
        data: { childId: updated.id, childName: updated.name, loanId: loan.id },
      });
    }
    setChild(updated);
  };
  ...
  const handlePartialRepayLoan = (loan) => {
    ...
    if (updated.parentId) {
      (typeof navigator === 'undefined' || !navigator.onLine) && addNotification(updated.parentId, {
        type: 'loan',
        title: isFullRepay ? '대출 상환 완료' : '대출 일부 상환',
        message: `${updated.name}님이 ${repayAmount.toLocaleString()}원을 상환했습니다.`,
        dedupeKey: isFullRepay
          ? `loan-repaid-parent:${updated.id}:${loan.id}`
          : `loan-partial-parent:${updated.id}:${loan.id}:${Date.now()}`,
        data: { childId: updated.id, childName: updated.name, loanId: loan.id },
      });
    }
    setChild(updated);
      ...
      },
      {
        ...
        extra: (
          <>
            ...
            <LinearProgress
              variant="determinate"
              value={progressTargetMissions > 0 ? (completedMissions / progressTargetMissions) * 100 : 0}
              sx={{
                height: 10,
                borderRadius: 5,
                mt: 1,

...
export default function ChatbotWidget({ userContext }) {
  ...
  const getLocalAnswer = (question) => {
    if (/부모/.test(question) || /아빠/.test(question) || /엄마/.test(question) || /아버지/.test(question) || /어머니/.test(question)) {
      return userContext.parentName
        ? `${userContext.parentName}님입니다!`
        : '연결된 부모님 정보를 찾을 수 없습니다.';
    }
    if (/안녕/.test(question) || /ㅎㅇ/.test(question) || /하이/.test(question) || /이름/.test(question)) {
      return `안녕하세요! ${userContext.name}님!`;
    }
    if (/잔액/.test(question)) {
